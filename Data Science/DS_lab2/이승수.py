...
for i in range(len(arr)): #mark dirty data
    if(int(arr[i][1])<140 or 200<int(arr[i][1])): #only 140cm~200cm 
        arr[i][1]=heightVal
        countH=countH+1
    if(150<int(arr[i][2]) or int(arr[i][2])<40): #only 40kg~150kg
        arr[i][2]=weightVal
        countW=countW+1
...
CDheight=[]
CDweight=[]
for i in range(len(arr)): #get clean dataset
...
estimIndex=[]
for i in range(len(arr)): #get outlier's indexs
...
plt.xlim([135,210])
plt.ylim([40,160])
plt.title('Scatter plot:linear regression estimation')
plt.show()
