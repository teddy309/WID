...
Dm=readDt.iloc[:,0:3]#get 'Gender','Height','Weight' columns
#select by columns and put Gender,Height,Weight column into dG,lX,lY
dG=np.array(Dm['Gender'])
lX=np.array(Dm['Height']).tolist()
lY=np.array(Dm['Weight']).tolist()
BMI=np.array([])
#get only female datas into dX,dY from lX,lY
dX=np.array([])
dY=np.array([])
for i in range(len(dG)):
    if dG[i]=="Male":#if 'Gender' column is 'Male', append to array dX,dY
...
plt.title("histogram of z-scores of male")
plt.xlabel('z_scores')
