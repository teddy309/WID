...
#select by rows and put Height,Weight column into dX,dY
D=readDt.iloc[:,0:3]#get 'Gender','Height','Weight' columns
dX=np.array(D['Height'])
dY=np.array(D['Weight'])#dY=w
BMI=np.array([])
...
plt.title("histogram of z-scores of both gender")
plt.xlabel('z_scores')
