...
var delay = 100;//santa delay
var santaX = 0.0;//santa Offset X,Y
var santaY = 0.0;
var sumOffset=vec2(0.0,0.0);

var santa_state = 0; //0 : nothing , 1: moving
var santa_center;// somewhere of santa: vec2(vertex[1140], vertex[1141])
       ...
       var smoke_vertex = new Float32Array([
         ...
         if(star_flag == 0) {
          ...
          //move Santa
               ...
               for (var i = 285 *4; i < 450 * 4; i++) {//scale&move santa 
                   if (i % 4 == 0) {//x
                       vertex[i] += 0.5 ;//initial Xpos=0.5
                       vertex[i] *= size;
                       vertex[i] += santaX;//add offset_X

                   }
                   else if (i % 4 == 1) {//y
                       vertex[i] -= 1.8;//initial Ypos=-1.8
                       vertex[i] *= size;
                       vertex[i] +=  santaY;//add offset_Y
   ...
   document.getElementById( "sinkingButton" ).onclick = function () {
    ...
    canvas.addEventListener("mousedown", function (event) {
        ...
        if (santa_state == 0) {
            ...
            var offset=vec2(0.0,0.0);//tmpx,y offset으로 바꿨어요 
            offset=vec2(t[0]-santa_center[0],t[1] - santa_center[1]);

            //TODO: change xDir here 

            console.log('santa_center x = ', santa_center[0]);
            console.log('santa_center y = ', santa_center[1]);
            console.log('santaX = ', offset[0]);
            console.log('santaY = ', offset[1]);

            var transPerAnimX=offset[0]*/iter;
            var transPerAnimY=offset[1]*/iter;
            for (i = 1; i <= iter; i++) {//iter=5
                santaX = offset[0] * i/iter;
                santaY = offset[1] * i/iter;
                vertex = setVertices(0);

                
                sumOffset=vec2(sumOffset[0]+transPerAnimX,sumOffset[1]+transPerAnimY);
                console.log('sumOffset= ',sumOffset);

                gl.changeBuffer("vPosition", vertex);
                sleep(100);//delay 0.1 sec per anim frame
